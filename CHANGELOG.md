...
### Added
- **WordPress REST driver** (`DriverEnum.WORDPRESS`): new driver targeting the [WordPress REST API](https://developer.wordpress.org/rest-api/) — the JSON API shipped with every WordPress install since 4.7.
...
- **Payload driver** (`DriverEnum.PAYLOAD`): new driver targeting [Payload](https://payloadcms.com/) — the TypeScript-native headless CMS / application framework; the response envelope is `mongoose-paginate-v2`, so the driver also fits any Express/Mongoose backend using that plugin.
...
- **Payload `FilterOperatorEnum` mapping**: `EQ`→`equals`, `GT`/`GTE`/`LT`/`LTE`→`greater_than`/`greater_than_equal`/`less_than`/`less_than_equal`, `CONTAINS`→`contains`, `ILIKE`→`like`, `IN`→`in` (CSV), `BTW`→`greater_than_equal`+`less_than_equal` pair (arity-checked), `NOT`→`not_equals` (single) / `not_in` (multi, CSV), `NULL`→`exists` with **inverted** boolean (`true` → `exists=false` ⇔ IS NULL; arity/type-checked). `SW` (no starts-with operator) and the PostgREST-only `FTS`/`PHFTS`/`PLFTS`/`WFTS` throw `UnsupportedFilterOperatorError`.
- **`PayloadResponseOptions`** (new class): pre-configured key mapping for the `mongoose-paginate-v2` envelope (`data → 'docs'`, `total → 'totalDocs'`, `perPage → 'limit'`, `lastPage → 'totalPages'`, `currentPage → 'page'`, `from → 'pagingCounter'`); derived/URL fields default to empty paths; all keys overridable via `IPaginationConfig`.

- **PocketBase driver** (`DriverEnum.POCKETBASE`): new driver targeting [PocketBase](https://pocketbase.io/) — the open-source single-binary backend (SQLite + realtime + auth).
...
- **PocketBase `FilterOperatorEnum` mapping**: `EQ`→`field='v'`, `GT`/`GTE`/`LT`/`LTE`→`>`/`>=`/`<`/`<=`, `CONTAINS`/`ILIKE`→`field~'v'` (PocketBase's `~` is case-insensitive and auto-wraps in `%...%`), `SW`→`field~'v%'` (explicit wildcard), `IN`→OR group, `BTW`→AND group `(field>=min && field<=max)` (arity-checked), `NOT`→`!=` (single) / AND-ed `!=` group (multi), `NULL`→`field=null`/`field!=null` (boolean dispatch, arity/type-checked). PostgREST-only `FTS`/`PHFTS`/`PLFTS`/`WFTS` throw `UnsupportedFilterOperatorError`.
- **`PocketbaseResponseOptions`** (new class): pre-configured key mapping for the records-list envelope (`data → 'items'`, `total → 'totalItems'`, `lastPage → 'totalPages'`, `currentPage → 'page'`, `perPage → 'perPage'`); derived/URL fields default to empty paths; all keys overridable via `IPaginationConfig`.
