...
- **Build and test targets moved to `@angular/build`** (#79): `angular.json` now uses `@angular/build:ng-packagr` and `@angular/build:karma` instead of the `@angular-devkit/build-angular` equivalents, which Angular 22 deprecates as part of its Webpack support deprecation and which printed a notice on every build. `@angular/build` exposes no `plugins/karma` entry point, so `karma.conf.js` drops the `@angular-devkit/build-angular` framework entry and its plugin `require`; `@angular-devkit/build-angular` is removed from `devDependencies` entirely in favour of `@angular/build`. Build-tooling only — no effect on the published package. Verified with the devkit fully uninstalled: build exit 0 with no deprecation notice, 985 specs passing, and coverage still written to `coverage/ng-qubee/lcov.info` (the path `ci.yml` hands to codecov) at 95.47% statements.
