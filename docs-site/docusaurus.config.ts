  ...
  plugins: [
    [
      'docusaurus-plugin-typedoc',
      {
        ...
        // Drop the per-file H1 ("Variable: NG\_QUBEE\_DRIVER", etc.).
        // typedoc-plugin-markdown escapes underscores in identifiers as
        // `\_` so they're not interpreted as italic markers, which leaks
        // into Docusaurus's autogenerated sidebar (the label is read from
        // the H1). With the title hidden, Docusaurus falls back to the
        // filename — `NG_QUBEE_DRIVER` — which has no escapes.
        hidePageTitle: true,
        sidebar: {
