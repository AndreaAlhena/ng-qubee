...
`paginationHeaders()` returns `null` for all drivers other than PostgREST in RANGE mode, so spreading the result is safe even on the API Platform / Directus / Feathers / Spatie / NestJS / @nestjsx/crud / JSON:API / json-server / Laravel / OData / Sieve / Strapi / Spring / DRF path.
